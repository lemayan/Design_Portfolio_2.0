...
export default function ContactHandset({children,reduced}:{children:ReactNode;reduced:boolean}){const [view,setView]=useState<PhoneView>('front'),[ready,setReady]=useState(false),[settled,setSettled]=useState(true),[awake,setAwake]=useState(false),[failed,setFailed]=useState(false),[zoom,setZoom]=useState(110),[isMobile,setIsMobile]=useState(false);const stage=useRef<HTMLDivElement>(null);
 useEffect(()=>{const element=stage.current!;const observer=new ResizeObserver(()=>setZoom(Math.min(element.clientWidth/3.15,element.clientHeight/6.2)));observer.observe(element);const mq=matchMedia('(max-width:760px)');const check=()=>setIsMobile(mq.matches);check();mq.addEventListener('change',check);return()=>{observer.disconnect();mq.removeEventListener('change',check);};},[]);
 ...
 {!failed&&<div className={'handset-canvas'+(ready?' ready':'')}><Boundary onError={()=>{setFailed(true);setReady(false);setView('front');setSettled(true);}}><Scene view={view} reduced={reduced} onReady={()=>setReady(true)} onSettled={()=>setSettled(true)} onPower={()=>setAwake(a=>!a)}/></Boundary></div>}
