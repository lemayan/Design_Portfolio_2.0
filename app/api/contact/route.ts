...
export async function POST(request:NextRequest){
 const allowedHost=process.env.CONTACT_ALLOWED_HOST;
 if(allowedHost&&new URL(request.url).hostname!==allowedHost)return NextResponse.json({error:'Email delivery is available on the production website.'},{status:503});
 ...
 const raw=await readLimitedBody(request);if(raw===null||raw.length>12000)return NextResponse.json({error:'This message is too long.'},{status:413});
 ...
 const ip=request.headers.get('cf-connecting-ip')||request.headers.get('x-forwarded-for')?.split(',')[0]?.trim()||'local';const record=hits.get(ip)||{count:0,expires:now+600000};if(record.count>=5)return NextResponse.json({error:'A few too many messages. Please try again in ten minutes.'},{status:429});record.count++;hits.set(ip,record);
